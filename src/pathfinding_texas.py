...
# Load road network
DATA_PATH = "./data/austin-dallas/austin_dallas.geojson"
...
plt.imshow(combined_raster, cmap="grey", origin="upper")
plt.title("Raster Bitmap (Roads/Rail + Environmentally Protected Areas)")
plt.colorbar(label="Raster Value")
plt.show()

def visualize_combined_raster(raster, protected_bitmap, path=None):
    combined_rgb = np.zeros((raster.shape[0], raster.shape[1], 3), dtype=np.uint8)

    # Background: white
    combined_rgb[:, :] = [255, 255, 255]

    # Roads: grey/black
    combined_rgb[raster == 1] = [128, 128, 128]

    # Goal points: blue
    combined_rgb[raster == 2] = [0, 0, 255]

    # Protected areas: green (overrides roads)
    combined_rgb[protected_bitmap == 3] = [0, 255, 0]

    # Path: red
    if path:
        for (x, y) in path:
            combined_rgb[x, y] = [255, 0, 0]

    plt.imshow(combined_rgb, origin='upper')
    plt.title("Raster Visualization (Roads, Goals, Protected Areas, and Path)")
    plt.axis("off")
    plt.show()

# Call the function with the path
visualize_combined_raster(raster, protected_bitmap)
