#!/usr/bin/env python3
"""
Plain NAMOA* Pathfinding using real cost maps (construction, environmental, geometry).

This is standard NAMOA* WITHOUT delayed reopening (no "dr").
Difference from NAMOA*-dr: when a new label is dominated by G_closed,
it is permanently discarded instead of being deferred for later re-check.
This makes it faster but may miss some Pareto-optimal solutions.
...
Usage:
    python namoa_plain_costmap.py
    python namoa_plain_costmap.py --output ./my_output --epsilon 0.15
"""
...
def namoa_plain_costmap(start, goals, cost_maps, heuristic_maps,
                        frame_interval=FRAME_RECORD_INTERVAL,
                        max_expansions=MAX_EXPANSIONS,
                        allow_diagonal=ALLOW_DIAGONAL,
                        epsilon=0.0):
    """
    Plain NAMOA* (NO delayed reopening) with real cost map objectives.

    Key difference from NAMOA*-dr:
    - When a new label is dominated by G_closed, it is permanently discarded.
    - No delayed set, no periodic reopening.
    - Faster but may miss some Pareto-optimal solutions.
    """
    ...
    heap = []
    tie_breaker = 0
    # NO delayed set — this is the key difference from NAMOA*-dr
    ...
    print("\nRunning plain NAMOA* (no delayed reopening) with real cost map objectives...")
    ...
    while heap:
        ...
        for neighbor, edge_cost in get_successors_multiobjective(current, cost_maps, allow_diagonal):
            ...
            if is_dominated_by_set(new_g, G_open[neigh_state], epsilon):
                continue
            # Plain NAMOA*: if dominated by G_closed, discard permanently (no delayed reopening)
        ...
        # NO delayed reopening block — labels dominated by G_closed are gone
...
def save_output_data(output_dir, road_bitmap, path, frames_sparse, metadata, pareto_paths=None):
    ...
    meta = {
        ...
        "algorithm": "NAMOA* (plain, cost-map objectives)",
...
def export_pareto_front(output_dir, pareto_paths, selected_path=None):
    ...
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump({"algorithm": "NAMOA* (plain, cost-map)",
...
def save_pareto_visualization(output_dir, construction_map, environmental_map, geometry_map,
    ...
    plt.suptitle(f'Plain NAMOA* Multi-Objective Pathfinding with Real Cost Maps\n'
                 f'{len(pareto_paths)} Pareto-optimal solutions found',
                 fontsize=16, fontweight='bold')
    plt.tight_layout()
    out_path = os.path.join(output_dir, "namoa_plain_costmap_pareto.png")
...
def main():
    parser = argparse.ArgumentParser(description="Plain NAMOA* (no dr) with real cost map objectives")
    ...
    if args.output is None:
        args.output = os.path.join(os.path.dirname(__file__), "namoa_plain_costmap_austin")

    print("=" * 70)
    print("Plain NAMOA* (no delayed reopening) with Real Cost Maps")
    print("=" * 70)
    ...
    print("\n" + "=" * 70)
    path, frames_sparse, pareto_paths = namoa_plain_costmap(
    ...
    if not args.no_animation and path is not None:
        ...
        try:
            ...
            animator.create_animation(output_file="namoa_plain_costmap_animation.gif",
                                      fps=args.fps, target_frames=args.frames,
                                      show_path=True, figsize=(12, 10), dpi=100)
            animator.create_static_comparison(output_file="namoa_plain_costmap_comparison.png",
                                              figsize=(18, 6), dpi=150)
            print(f"  Animation: {args.output}/namoa_plain_costmap_animation.gif")
            print(f"  Comparison: {args.output}/namoa_plain_costmap_comparison.png")
